...
# implémentation tensorboard (si besoin) -------------------------------------------------
## si bug tensorboard : 
...
for class_index in range(nb_class):
    ...
    plt.plot(recall, precision, marker='.', label=f'Classe {class_names[class_index]}')

plt.xlabel('Rappel')
plt.ylabel('Précision')
plt.title('Courbe Précision-Rappel pour chaques classes')
plt.legend()
plt.grid()
plt.show()

# Graphique précision en fonction seuil
plt.figure(figsize=(16, 60))
for class_index in range(nb_class):
    precision, recall, thresholds = precision_recall_curve(y_test_true == class_index, y_test_pred_prob[:, class_index])
    
    plt.subplot(2, 1, 1)  
    plt.plot(precision[:-1], thresholds, marker='.', label=f'Classe {class_names[class_index]}')  # precision[:-1] car il y a un point de moins que les seuils
    plt.xlabel('Précision', fontsize=40)
    plt.ylabel('Seuil', fontsize=40)     
    plt.title('Précision en fonction du seuil', fontsize=50) 
    plt.legend(fontsize=30) 
    plt.grid()
    plt.gca().invert_yaxis()
plt.show()

# Graphique rappel en fonction seuil
plt.figure(figsize=(24, 16))
for class_index in range(nb_class):
    precision, recall, thresholds = precision_recall_curve(y_test_true == class_index, y_test_pred_prob[:, class_index])

    plt.subplot(2, 1, 2)  
    plt.plot(thresholds, recall[:-1], marker='.', label=f'Classe {class_names[class_index]}')  # recall[:-1] car il y a un point de moins que les seuils
    plt.xlabel('Seuil', fontsize=30)  
    plt.ylabel('Rappel', fontsize=30)  
    plt.title('Rappel en fonction du seuil', fontsize=40)  
    plt.legend(fontsize=30) 
    plt.grid()
plt.show()
...
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('Taux de faux positifs', fontsize=30)
plt.ylabel('Taux de vrais positifs', fontsize=30)
plt.title('Courbes ROC par classe', fontsize=30)
...
plt.legend()
plt.ylim(0, 1)  # car score F1 est entre 0 et 1
plt.show()
